    ...
    <Layout>
      ...
      <section className="max-w-[1400px] mx-auto" aria-label="Clients management">
        <header className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0 mb-6 sm:mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-semibold text-gray-800 m-0 mb-2 tracking-tight">Clients</h1>
            <p className="text-sm sm:text-base text-gray-600 m-0">Manage your client relationships and information</p>
          </div>
          <AccessibleButton
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6" role="list" aria-label="Client list">
            {clients.map((client) => (
              <article 
                key={client._id}
                role="listitem"
